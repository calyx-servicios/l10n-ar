...
class L10nArDjArba(models.Model):
    ...
    def _compute_display_name(self):
        ...
        for rec in to_compute:
            ...
            n_fortnight = _("1st") if rec._get_fortnight(rec.date) == 1 else _("2nd")
            rec.display_name = _("Period %s %s - %s Fortnight", name_month, rec.date.year, n_fortnight)
    ...
    def _create_withholding(self, wh_line):
        ...
        """
        # Resuelve el documento fuente (payment_group o payment individual).
        # En Odoo 16, wh_line es directamente un account.payment.
        wh_partner = wh_line.payment_group_id.partner_id or wh_line.partner_id
        wh_company = wh_line.payment_group_id.company_id or wh_line.company_id
        wh_date = wh_line.payment_group_id.payment_date if wh_line.payment_group_id else wh_line.date
        wh_doc = wh_line.payment_group_id or wh_line

        if not self:
            self = self._ensure_dj(wh_date, wh_company)
        ...
        request_data = {
            "idDj": self.name,
            "cuitContribuyente": int(wh_partner.ensure_vat()),
            "razonSocialContribuyente": wh_partner.name,
            "cuitAgente": int(self.company_id.partner_id.ensure_vat()),
            "sucursal": 1,  # TODO revisar que valor string <= 5, debiendo ser numéricos
            "alicuota": wh_line.tax_withholding_id.amount,
            "baseImponible": wh_line.withholding_base_amount,
            "importeRetencion": wh_line.amount,
            "fechaOperacion": fields.Datetime.now().strftime(
                "%Y-%m-%dT%H:%M:%S.000Z"
            ),  # wh_line.date.strftime("%Y-%m-%dT%H:%M:%S.000Z")
            "nTransaccionAgente": re.sub("[^0-9]", "", wh_line.withholding_number or ""),  # Obligatorio String(20)
        }
        response, error = self._process_arba_response(
            "POST", "/comprobante", env_type, "Enviar Retencion a ARBA", request_data
        )
        if error:
            wh_doc.message_post(body="Error al enviar retencion a ARBA: " + error)
            return
